...
<head>
    ...
    <title>Return Orders - Esang Delicacies</title>
...
<body>
        ...
        <div class="container">
            ...
            <div class="page-header">
                <div class="row align-items-center">
                    <div class="col-md-8">
                        <h1 class="mb-2">
                            <i class="fas fa-undo me-3"></i>Return Orders
                        </h1>
                        <p class="mb-0 opacity-75">
                            View all cancelled and returned orders from the database
                        </p>
                    </div>
                    <div class="col-md-4 text-end">
                        <button class="btn btn-light" onclick="refreshReturnOrders()">
            ...
            <div class="row">
                <div class="col-12">
                    <div class="loading-spinner" id="loadingSpinner">
                        ...
                        <p class="mt-2">Loading return orders from database...</p>
                    </div>
    ...
    <script>
        ...
        function refreshReturnOrders() {
            ...
            fetch('api/admin_orders.php?action=get_orders&status=cancelled,returned')
                .then(response => response.json())
                .then(data => {
                    hideLoading();
                    if (data.success) {
                        displayReturnOrders(data.orders);
                    } else {
                        showError(data.message || 'Failed to load return orders');
                    }
                })
                .catch(error => {
                    hideLoading();
                    console.error('Error loading return orders:', error);
                    showError('Error loading return orders from database. Please try again.');
                });
        }

        function displayReturnOrders(orders) {
            const container = document.getElementById('returnOrdersList');
            ...
            if (orders.length === 0) {
                container.innerHTML = `
                    <div class="empty-state">
                        ...
                        <p>No cancelled or returned orders found in the database.</p>
                    </div>
                `;
                return;
            }

            const orderCards = orders.map(order => createReturnOrderCard(order)).join('');
            container.innerHTML = orderCards;
        }

        function createReturnOrderCard(order) {
            const statusColor = order.status === 'cancelled' ? 'danger' : 'warning';
                ...
                <div class="order-card">
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-4 text-end">
                                <div class="mb-3">
                                    <div class="h4 text-danger">₱${parseFloat(order.total_amount || 0).toFixed(2)}</div>
        ...
        function showError(message) {
            document.getElementById('returnOrdersList').innerHTML = `
        ...
        setInterval(refreshReturnOrders, 30000);
    </script>
</body>
</html>