class OrderManagerStatus {
    ...
    initializeSyncService() {
        if (typeof OrderSyncService !== 'undefined') {
            this.syncService = new OrderSyncService({
                pollType: 'order_manager',
                pollInterval: 15000, // 15 seconds for order manager updates
                onStatusUpdate: (update) => this.handleStatusUpdate(update),
                onError: (error) => this.handleSyncError(error),
                onConnect: () => console.log('Order Manager sync service connected')
            });
        } else {
    ...
    renderOrdersTable() {
        ...
        tableBody.innerHTML = this.orders.map(order => `
            <tr class="order-row" data-order-id="${order.order_id}">
                ...
                <td class="rider-info">
                    ${order.has_rider ? `
                        <span class="rider-name">${this.escapeHtml(order.rider_name)}</span>
                        <span class="rider-status assigned">✓ Assigned</span>
                    ` : `
                        <span class="rider-status ${order.rider_status === 'Pending Assignment' ? 'pending' : 'not-required'}">
                            ${order.rider_status === 'Pending Assignment' ? '⏳ Pending' : '—'}
                        </span>
                    `}
                </td>
                ...
                <td class="actions">
                    <div class="action-buttons">
                        ...
                        ${order.status !== 'cancelled' ? `
                            <button class="btn btn-danger btn-sm" onclick="orderManager.updateOrderStatus(${order.order_id}, 'cancel')">
                ...
                <div class="order-detail-section">
                    ...
                    <div class="detail-grid">
                        <div class="detail-item">
                            ...
                            <span>${this.escapeHtml(order.rider_name)}</span>
                        </div>
                        ${order.rider_phone ? `
                            <div class="detail-item">
                                <label>Rider Phone:</label>
                                <span>${this.escapeHtml(order.rider_phone)}</span>
                            </div>
                        ` : ''}
                        ${order.rider_plate_number && order.rider_plate_number !== 'N/A' ? `
                            <div class="detail-item">
                                <label>Vehicle Plate:</label>
                                <span>${this.escapeHtml(order.rider_plate_number)}</span>
                            </div>
                        ` : ''}
                        <div class="detail-item">
                            <label>Assignment Status:</label>
                            <span class="rider-status-badge">Assigned</span>
                        </div>
                    </div>
                </div>
            ` : (order.status === 'out_for_delivery' || order.status === 'delivered' ? `
                <div class="order-detail-section">
                    ...
                    <div class="detail-grid">
                        <div class="detail-item">
                            <label>Assignment Status:</label>
                            <span class="rider-status-badge pending">Pending Assignment</span>
                        </div>
                    </div>
                </div>
            ` : '')}
    ...
    handleStatusUpdate(update) {
        console.log('Order Manager received order update:', update);
    ...
    handleSyncError(error) {
        console.error('Order Manager sync error:', error);
...
document.addEventListener('DOMContentLoaded', function() {
    orderManager = new OrderManagerStatus();
});